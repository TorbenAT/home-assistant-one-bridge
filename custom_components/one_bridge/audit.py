...
_AUDIT_CONTEXT: ContextVar[dict[str, Any]] = ContextVar(
    "one_bridge_audit_context", default={}
)
...
class AuditLog:
    ...
    @contextmanager
    def context(self, metadata: dict[str, str]):
        """Apply credential-free request metadata to nested audit entries."""
        token = _AUDIT_CONTEXT.set({**_AUDIT_CONTEXT.get(), **metadata})
        try:
            yield
        finally:
