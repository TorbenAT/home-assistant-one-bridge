...
def _https_url(value: Any, field: str) -> str:
    ...
    if (
        ...
        raise ValueError(f"{field} skal være en offentlig https-URL med hostname og uden credentials.")
    return text
...
def _public_base_url(value: Any) -> str:
    ...
    origin = _https_url(raw, "public_base_url")
    try:
        ipaddress.ip_address(urlparse(origin).hostname)
    except ValueError:
        return origin
    raise ValueError("public_base_url må ikke bruge en literal IP-adresse.")
